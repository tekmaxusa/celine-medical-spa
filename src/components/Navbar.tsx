        ...
        <div className="max-w-[1400px] mx-auto px-6 h-20 flex justify-between items-center">
          <Link to="/" className="flex items-center gap-3 min-w-0" aria-label="Celine Medical Spa home">
            <img
              ...
              className="h-9 w-auto max-h-10 sm:h-10 sm:max-h-11 object-contain shrink-0"
            />
          ...
          <div className="hidden md:flex items-center gap-10">
            {navItems.map((item) => (
              <Link 
                key={item.name} 
                to={item.path} 
                className={`text-sm uppercase tracking-[0.1em] font-bold transition-colors ${
            ...
            <a href="tel:9727506100" className="px-6 py-2.5 bg-terracotta text-pure text-xs uppercase tracking-[0.1em] font-bold rounded-[2px] hover:bg-sunset transition-all shadow-sm">
              (972) 750-6100
            </a>
          </div>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed inset-0 top-20 z-40 bg-warm-light flex flex-col p-6 border-b border-line"
          >
